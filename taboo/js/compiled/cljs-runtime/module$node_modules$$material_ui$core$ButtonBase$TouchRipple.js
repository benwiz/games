...
Math.round(clientY-rect.top)}if(_options$pulsate){var rippleSize=Math.sqrt((2*Math.pow(rect.width,2)+Math.pow(rect.height,2))/3);0===rippleSize%2&&(rippleSize+=1)}else rippleSize=Math.sqrt(Math.pow(2*Math.max(Math.abs((options?options.clientWidth:0)-rippleX),rippleX)+2,2)+Math.pow(2*Math.max(Math.abs((options?options.clientHeight:0)-rippleY),rippleY)+2,2));event.touches?null===startTimerCommit.current&&(startTimerCommit.current=function(){startCommit({pulsate:pulsate,rippleX:rippleX,rippleY:rippleY,
rippleSize:rippleSize,cb:cb})},startTimer.current=setTimeout(function(){startTimerCommit.current&&(startTimerCommit.current(),startTimerCommit.current=null)},80)):startCommit({pulsate:pulsate,rippleX:rippleX,rippleY:rippleY,rippleSize:rippleSize,cb:cb})}},[centerProp,startCommit]),pulsate$jscomp$0=React.useCallback(function(){start({},{pulsate:!0})},[start]),stop=React.useCallback(function(event,cb){clearTimeout(startTimer.current);"touchend"===event.type&&startTimerCommit.current?(event.persist(),
startTimerCommit.current(),startTimerCommit.current=null,startTimer.current=setTimeout(function(){stop(event,cb)})):(startTimerCommit.current=null,setRipples(function(oldRipples){return 0<oldRipples.length?oldRipples.slice(1):oldRipples}),rippleCallback.current=cb)},[]);React.useImperativeHandle(ref,function(){return{pulsate:pulsate$jscomp$0,start:start,stop:stop}},[pulsate$jscomp$0,start,stop]);return React.createElement("span",(0,_extends2.default)({className:(0,_clsx.default)(classes.root,_props$center),
ref:container},props),React.createElement(_reactTransitionGroup.TransitionGroup,{component:null,exit:!0},ripples))});_interopRequireDefault.propTypes={center:global.default.bool,classes:global.default.object.isRequired,className:global.default.string};require=(0,module.default)(require,{flip:!1,name:"MuiTouchRipple"})(React.memo(_interopRequireDefault));exports.default=require}
